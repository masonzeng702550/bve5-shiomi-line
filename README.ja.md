...
`Shiomi/objects/` には**自動生成の仮素材（プレースホルダ）**を同梱しています。
全ストラクチャが灰色の1m立方体、全サウンドが0.1秒の無音で、
**そのままでもエラーなく読み込め、端から端まで運転できます**（見た目は灰色の箱）。
実際の風景を得るには各 `.x` / `.wav` を本物のモデル・音声に差し替えてください
（配置は structures.txt / sounds.txt のパスに従います）。車両は引き続き
**同梱していません**（STEP 3 参照）。詳細は `Shiomi/objects/PLACEHOLDER.txt` と `CREDITS.txt`。
